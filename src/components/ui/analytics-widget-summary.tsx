...
type Props = {
  ...
  // kept minimal: only mini bars and sparkline are supported
...
export default function AnalyticsWidgetSummary({
  ...
  return (
    ...
    >
      ...
      <div 
        className={cn("absolute top-4 right-4 flex items-center gap-1.5 px-3 py-1.5 rounded-full backdrop-blur-sm shadow-sm" , {
      ...
      <div className="relative flex items-end gap-4 justify-between flex-wrap">
        ...
        <div className="w-24 h-16 flex items-end gap-1">
          {chartData && chartType === 'sparkline' ? (
            ...
            <ApexSparkline series={chartData as any} dataKey={dataKey} color={chartColor || color} />
          ) : chartData && chartType === 'mini-bars' ? (
            // @ts-ignore
            <ApexMiniBarChart data={chartData as any} dataKey={dataKey} color={chartColor || color} height={56} />
          ) : null}
        </div>
      </div>
